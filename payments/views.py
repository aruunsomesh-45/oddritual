...
def process_checkout(request):
    ...
    # Query actual order and invoice data
    try:
        order = Razorpay_order.objects.get(order_id=order_id)
        inv = invoice.objects.get(razorpay_order=order)
        products = [
            {'name': p.product_name, 'qty': 1, 'price': float(p.price)}
            for p in inv.products.all()
        ]
    except (Razorpay_order.DoesNotExist, invoice.DoesNotExist):
        return JsonResponse({'error': 'Order not found'}, status=404)
    
    if not products:
        return JsonResponse({'error': 'No products found for this order'}, status=400)
